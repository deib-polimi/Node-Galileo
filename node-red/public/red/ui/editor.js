...
RED.editor = (function() {
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        if (editing_node) {
                        ...
                        } else if (RED.view.state() == RED.state.CONFIG_DEVICES) {
                            var devices = Array();
                            var elements = document.getElementsByName("device-config-element");
                            for(var i = 0; i < elements.length; i++) {
                                //console.log($("input[name='device-input-name']")[i]);
                                var device = {
                                    "name": $("#device-input-name-"+i).val(),
                                    "deviceAddress": $("#device-input-address-"+i).val(),
                                    "ID": $("#device-input-id-"+i).val(),
                                    "is_server": $("#device-input-is_server-"+i).val() === "true"
                                };
                                devices.push(device);
                            }
                            //console.log(devices);
                            ...
                            devices.forEach(function(device) {
                                if(device.ID != RED.currentDevice.ID && (device.ID === undefined || device.ID == null || device.ID == "")) {
                                    var call = $.ajax({
                                        url:"getDeviceID",
                                        type: "POST",
                                ...
                                try {
                                    if(calls.length == 1) {
                                        if(typeof arguments[0] == 'string') {
                                            ...
                                            //console.log(json);
                                            for(j in devices) {
                                                if(devices[j].deviceAddress == json.deviceAddress) {
                                                    devices[j].ID = json.ID;
                                                    devices[j].is_server = json.is_server;
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    console.log(error);
                                    RED.notify("<strong>Error</strong>: unable to connect to "+device.name,"error");
                                }
                                RED.view.setDevices(devices);
                            }).fail(function() {
                                console.log('error');
                                console.log(arguments);
                                RED.notify("<strong>Error</strong>: unable to connect to "+device.name,"error");
                            });
                        } else if (RED.view.state() == RED.state.EXPORT_MACRO_NODE) {  
                            var MacroNode = {};
                            MacroNode.name = $("#macronode-name").val();
                            if(MacroNode.name.trim() == "" || MacroNode.name === undefined || MacroNode.name === null) {
                                RED.notify("<strong>Error</strong>: enter a macronode name","error");
                                return;
                            }
                            for(var i = 0; i < RED.macroNodes.length; i++) {
                                var n = RED.macroNodes[i];
                                if(n.name == MacroNode.name) {
                                    RED.notify("<strong>Error</strong>: a macronode with this name already exists","error");
                                    return;
                                }
                            }
                            var numOfInputs = $("#macronode-num-inputs").val();
                            var input = null;
                            if(numOfInputs == 1)
                                input = $("#macronode-inputNode-"+0).val();
                            /*var inputs = Array();
                            for(var i = 0; i < numOfInputs; i++) {
                                inputs.push($("#macronode-inputNode-"+i).val());
                            }*/
                            var numOfOutputs = $("#macronode-num-outputs").val();
                            var outputs = Array();
                            for(var i = 0; i < numOfOutputs; i++) {
                                var val = $("#macronode-outputNode-"+i).val().split("-");
                                outputs.push({
                                    "id":val[0],
                                    "output":val[1]
                                });
                            }
                            MacroNode.inputNode = input;
                            //MacroNode.inputNodes = inputs;
                            MacroNode.inputs = parseInt(numOfInputs);
                            MacroNode.outputNodes = outputs;
                            MacroNode.outputs = parseInt(numOfOutputs);
                            var tmpNodes = $("#macronode-nodes-list").val();
                            MacroNode.nodes = JSON.parse(tmpNodes);
                            /*var minX, minY;
                            for(var i=0;i<MacroNode.nodes.length;i++) {
                                var n = MacroNode.nodes[i];
                                if(!minX)
                                    minX = n.x;
                                if(!minY)
                                    minY = n.y;
                            ...
                            //console.log(JSON.stringify(MacroNode));
                            RED.view.addMacroNode(MacroNode);
                        }
                        $( this ).dialog( "close" );
                    }
                },
            ...
            open: function(e) {
                ...
                if (editing_node) {
                    ...
                    if(RED.view.InsideMacronode()) {
                        ...
                        $('#node-input-deviceID').attr('disabled', 'disabled');
                    }
                }
            },
            close: function(e) {
    ...
    function showEditDialog(node) {
        ...
        //if(node.generic_type === undefined)
            $("#dialog-form").html($("script[data-template-name='"+node.type+"']").html());
        //else
            //$("#dialog-form").html($("script[data-template-name='"+node.generic_type+"']").html());
