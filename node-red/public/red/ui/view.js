...
RED.view = (function() {
    ...
    var space_width = 5000,
        ...
        scaleFactor = 1,
        node_width = 100,
        node_height_no_device = 30,
        node_height_device = 35,
        node_height = node_height_device;
    ...
    function updateSelection() {
        if (moving_set.length === 0) {
            ...
            RED.menu.setDisabled("btn-create-macronode",true);
        } else {
        ...
        if (moving_set.length == 1) {
            ...
            //drawMacronodePanel(moving_set[0].n);
            //console.log(moving_set[0]);
        } else {
    ...
    function deleteSelection() {
        ...
        if (moving_set.length > 0) {
            for (var i=0;i<moving_set.length;i++) {
                ...
                deleteMacroNodeChilds(node,removedLinks,removedNodes);
            }
            moving_set = [];
            setDirty(true);
        }
    ...
    function deleteMacroNodeChilds(node,removedLinks,removedNodes) {
        if(node.type == "MacroNode") {
            ...
            for(i=0;i<nodes.length;i++) {
                ...
                if(n.father == node.id) {
                    toRemove.push(n);
                    deleteMacroNodeChilds(n, removedLinks, removedNodes);
                }
            }
            for(i=0;i<toRemove.length;i++) {
                ...
                if(n.type == "MacroNode")
                    deleteMacroNodeChilds(n,removedLinks,removedNodes)
                var rmlinks = RED.nodes.remove(n.id);
                removedNodes.push(n);
                removedLinks = removedLinks.concat(rmlinks);
            }
        }
    ...
    function calculateDeviceTextWidth(str) {
        var sp = document.createElement("span");
        sp.className = "node_device";
        sp.style.position = "absolute";
        sp.style.top = "-1000px";
        sp.innerHTML = (str||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
        document.body.appendChild(sp);
    ...
    function redraw() {
        ...
        if(active_macronode[activeWorkspace] === undefined)
            ...
            node_height = node_height_no_device;

        if (mouse_mode != RED.state.JOINING) {
            // Don't bother redrawing nodes if we're drawing links
            //my code
            var node;
            if(/*macronode_state == RED.macroNodeState.CHILD &&*/ active_macronode[activeWorkspace] !== undefined && RED.nodes.node(active_macronode[activeWorkspace]).z == activeWorkspace)
                node = vis.selectAll(".nodegroup").data(RED.nodes.nodes.filter(function(d) { return d.father == active_macronode[activeWorkspace] }),function(d){return d.id});
            else
                node = vis.selectAll(".nodegroup").data(RED.nodes.nodes.filter(function(d) { return d.z == activeWorkspace && d.father === undefined }),function(d){return d.id});    
            //end
            node.exit().remove();

            var nodeEnter = node.enter().insert("svg:g").attr("class", "node nodegroup");
            nodeEnter.each(function(d,i) {
                    //var node_height = d._def.device ? 60 : 30;
                    ...
                    if(active_macronode[activeWorkspace] === undefined) {
                        dev = d._def.device;
                        dev = (typeof dev === "function" ? dev.call(d) : dev)||"";
                    }
                    d.w = Math.max(node_width,calculateTextWidth(l)+(d._def.inputs>0?7:0), calculateDeviceTextWidth(dev)+(d._def.inputs>0?7:0));
                    ...
                    /*if(d._def.deviceLabel) {
                        var nodeLabelGroup = node.append('svg:g')
                            .attr("transform",function(d) { return "translate(2,"+((d._def.align == "down") ? 94 : -25)+")"; })
                            .attr("class",function(d) { return "node_button "+((d._def.align == "right") ? "node_right_button" : "node_left_button"); });
                    }*/
                   ...
                   //node.append("rect").attr("class", "node-gradient-bottom").attr("rx", 6).attr("ry", 6).attr("height",30).attr("stroke","none").attr("fill","url(#gradient-bottom)").style("pointer-events","none");
                    ...
                    if(active_macronode[activeWorkspace] === undefined) {
                        var device = node.append('svg:text').attr('class','node_device').attr('x', 38).attr('dy', '.35em').attr('text-anchor','start');
                        if (d._def.align) {
                            device.attr('class','node_device node_label_'+d._def.align);
                            device.attr('text-anchor','end');
                        }
                    }
            ...
            node.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        //my code
                        if(d._def.device && active_macronode[activeWorkspace] === undefined) {
                            thisNode.selectAll('text.node_label').text(function(d,i){
                                ...
                                .attr('y', function(d){return (d.h/4);})
                            ...
                            thisNode.selectAll('text.node_device').text(function(d,i){
                                    if (d._def.device) {
                                        if (typeof d._def.device == "function") {
                                            return d._def.device.call(d);
                                        } else {
                                            return d._def.device;
                                        }
                                    }
                                    return "";
                            })
                                .attr('y', function(d){return (d.h/4)+(d.h/2)-1;})
                                .attr('class',function(d){
                                    return 'node_device'+
                                    (d._def.align?' node_label_'+d._def.align:'')+
                                    (d._def.labelStyle?' '+(typeof d._def.labelStyle == "function" ? d._def.labelStyle.call(d):d._def.labelStyle):'') ;
        ...
        var link;
        if(/*macronode_state == RED.macroNodeState.CHILD &&*/ active_macronode[activeWorkspace] !== undefined && RED.nodes.node(active_macronode[activeWorkspace]).z == activeWorkspace)
            link = vis.selectAll(".link").data(RED.nodes.links.filter(function(d) { return d.source.z == activeWorkspace && d.target.z == activeWorkspace && d.source.father == active_macronode[activeWorkspace] && d.target.father == active_macronode[activeWorkspace] }),function(d) { return d.source.id+":"+d.sourcePort+":"+d.target.id;});
            //node = vis.selectAll(".nodegroup").data(RED.nodes.nodes.filter(function(d) { return d.father == active_macronode[activeWorkspace] }),function(d){return d.id});
        else
            link = vis.selectAll(".link").data(RED.nodes.links.filter(function(d) { return d.source.z == activeWorkspace && d.target.z == activeWorkspace && d.source.father === undefined && d.target.father === undefined }),function(d) { return d.source.id+":"+d.sourcePort+":"+d.target.id;});
        //end
    ...
    function inspectMacroNode() {
        for(var n in moving_set) {
            var node = moving_set[n].n;
            if(node.type == "MacroNode") {
                //$('#goUpPanel').show();
                active_macronode[activeWorkspace] = node.id;
                //macronode_state = RED.macroNodeState.CHILD;
                /*var nodes = [];
    ...
    function drawMacronodePanel() {
        if(moving_set.length == 1 && moving_set[0].n !== undefined && moving_set[0].n.type == "MacroNode") {
            $('#inspectPanel').show();
        }
        else {
            $('#inspectPanel').hide();
            if(active_macronode[activeWorkspace] === undefined)
                ...
                $('#goUpPanel').show();
                var parent = RED.nodes.node(active_macronode[activeWorkspace]);
                if(parent.father === undefined || parent.father === null)
                    $('#goUpPanel').html("back to root");
                else {
                    parent = RED.nodes.node(parent.father);
                    var name = parent.name!=""?parent.name:RED.macroNodeSet[parent.templateID].name;
                    $('#goUpPanel').html("back to "+name);
                }
            }
        }
    }
    ...
    function goUp() {
        var n = RED.nodes.node(active_macronode[activeWorkspace]);
        if(n !== undefined) {
            if(n.father === undefined) {
                delete active_macronode[activeWorkspace];
                //macronode_state = RED.macroNodeState.DEFAULT;
                //$('#goUpPanel').hide();
            }
            else {
                active_macronode[activeWorkspace] = n.father;
            }
            clearSelection();
            RED.nodes.eachNode(function(n) {
                n.dirty = true;
    ...
    function showConfigDevicesDialog() {
        mouse_mode = RED.state.CONFIG_DEVICES;
        $("#dialog-form").html($("script[data-template-name='devices-manage-dialog']").html());
        RED.devices.forEach(function(device){
            $("#devices-list").append($("script[data-template-name='device-element']").html());
        });
        for(var i=0; i< RED.devices.length;i++) {
            var device = RED.devices[i];
            $("input[name='device-input-id']")[i].setAttribute('value', device.ID);
            $("input[name='device-input-id']")[i].setAttribute('id', 'device-input-id-'+i);
            $("input[name='device-input-name']")[i].setAttribute('value', device.name);
            $("input[name='device-input-name']")[i].setAttribute('id', 'device-input-name-'+i);
            $("input[name='device-input-address']")[i].setAttribute('value', device.deviceAddress);
            $("input[name='device-input-address']")[i].setAttribute('id', 'device-input-address-'+i);
            $("input[name='device-input-is_server']")[i].setAttribute('value', device.is_server);
            $("input[name='device-input-is_server']")[i].setAttribute('id', 'device-input-is_server-'+i);
            if(device.ID == RED.currentDevice.ID)
                $("span[name='device-remove-button']")[i].remove();
            $(document).on('click','.devices-remove-button',function(){
                $(this).parent().parent().remove();
                var elements = document.getElementsByName("device-config-element");
            ...
            /*$("span[name='device-remove-button']")[i].click(function() {
                console.log('click');
                //$(this).parent().parent().remove();
            });*/
        }
        ...
        //$("#devices-input-import").val(JSON.stringify(RED.devices, null, 4));
        $( "#dialog" ).dialog("option","title","Config Devices").dialog( "open" );
    }
    ...
    function showExportMacroNodeDialog() {
        mouse_mode = RED.state.EXPORT_MACRO_NODE;
        $("#dialog-form").html($("script[data-template-name='create-macronode-dialog']").html());
        //$("#macronode-inputNode").append($("<option></option>").val(null).text('no input'));
        $( "#macronode-num-inputs" ).spinner({
            min:0,
            max:1,
    ...
    function drawMacronodeInputsEditor(value) {
        var inputRows = document.getElementsByName('macronode-input-select-row');
        var nns = RED.nodes.createExportableNodeSet(moving_set);
        //console.log(nns);
        if(inputRows.length < value) {
            $("#macronode-input-list").append($("script[data-template-name='macronode-input-select']").html());
            var inputs = document.getElementsByName('macronode-inputNode');
            for(var i = 0; i < inputs.length; i++) {
                inputs[i].setAttribute('id', 'macronode-inputNode-'+i);
                if (i >= value-1)
                    for (var d in nns) {
                        var node = RED.nodes.node(nns[d].id);
                        //console.log(node);
                        if(node._def.inputs > 0 && node.father === active_macronode[activeWorkspace])
                          $('#macronode-inputNode-'+i).append($("<option></option>").val(node.id).text('['+node.type +'] '+node.name));
                    }
            }
            /*var labels = document.getElementsByName('macronode-input-label');
            for(var i = 0; i < labels.length; i++) {
                labels[i].value = "Input "+ (i + 1);
            }*/
    ...
    function drawMacronodeOutputsEditor(value) {
        var inputRows = document.getElementsByName('macronode-output-select-row');
        var nns = RED.nodes.createExportableNodeSet(moving_set);
        if(inputRows.length < value) {
            $("#macronode-output-list").append($("script[data-template-name='macronode-output-select']").html());
            var inputs = document.getElementsByName('macronode-outputNode');
            for(var i = value-1; i < inputs.length; i++) {
                inputs[i].setAttribute('id', 'macronode-outputNode-'+i);
                //if (i >= value-1)
                    for (var d in nns) {
                        var node = RED.nodes.node(nns[d].id);
                        if(node._def.outputs > 0 && node.father === active_macronode[activeWorkspace]) {
                            for(var o=0; o<node.outputs; o++)
                                if(node.outputs == 1)
                                    $('#macronode-outputNode-'+i).append($("<option></option>").val(node.id+'-'+o).text('['+node.type +'] '+node.name));
                                else
                                    $('#macronode-outputNode-'+i).append($("<option></option>").val(node.id+'-'+o).text('['+node.type +'] '+node.name + " - output:" + o));
                        }
                    }
            }
    ...
    function addMacroNode(value) {
        $.ajax({
            ...
            RED.notify("Successfully created","success");
        }).fail(function(xhr,textStatus,err) {
            console.log("error status code: " + xhr.status);
            //RED.view.dirty(true);
            RED.notify("<strong>Error</strong>: unable to create macro node","error");
            /*if (xhr.responseText) {
    ...
    function setDevices(json) {
        try {
            //var json = JSON.parse(devicesStr);
            json.forEach(function(device){
                if(device.ID != RED.currentDevice.ID) {
                    ...
                    $.ajax({
                        ...
                        //console.log("ok status code: " + xhr.status);
                        RED.notify("Successfully updated device list on " + device.name,"success");
                    }).fail(function(xhr,textStatus,err) {
                        console.log("error status code: " + xhr.status);
                        RED.view.dirty(true);
                        RED.notify("<strong>Error</strong>: unable to update device list on "+device.name,"error");
                        /*if (xhr.responseText) {
                            RED.notify("<strong>Error</strong>: "+xhr.responseText,"error");
                        } else {
                            RED.notify("<strong>Error</strong>: no response from server","error");
    ...
    return {
        ...
        //my code
        /*macroNodeState:function(state) {
            if (state == null) {
                return mouse_mode
            } else {
                macronode_state = state;
            }
        },*/
        //end
        ...
        showConfigDevicesDialog: showConfigDevicesDialog,
        setDevices: setDevices,
        showExportMacroNodeDialog: showExportMacroNodeDialog,
        addMacroNode: addMacroNode,
        //my code
        inspectMacroNode: inspectMacroNode,
        goUp: goUp,
        InsideMacronode: function() {
            return active_macronode[activeWorkspace] !== undefined;
        }
        //end
    };
})();
